...
ANALYST = {"user_id": "analyst_kerem", "role": "model_analyst",
           "branch_scope": "NONE"}
AUDITOR = {"user_id": "auditor_selin", "role": "auditor",
           "branch_scope": "ALL"}
...
# ---------------------------------------------------------------------------
# P-01.3 audit replay & separation of duties
# ---------------------------------------------------------------------------
def test_p01_auditor_production_evaluation_still_denied():
    d = ENGINE.evaluate(ctx(actor=AUDITOR))
    assert d.effect == "DENY" and d.matched_rule["rule_id"] == "P-01.1"


def test_p01_auditor_replay_allowed():
    d = ENGINE.evaluate(ctx(actor=AUDITOR, event_type="audit_replay"))
    assert d.effect != "DENY"
    assert not any(m["policy_id"] == "P-01" for m in d.all_matched)


def test_p01_analyst_replay_allowed_despite_empty_branch_scope():
    d = ENGINE.evaluate(ctx(actor=ANALYST, event_type="audit_replay"))
    assert d.effect != "DENY"          # P-01.2 exempted for replays


def test_p01_officer_cannot_use_replay_channel():
    d = ENGINE.evaluate(ctx(actor=OFFICER, event_type="audit_replay"))
    assert d.effect == "DENY" and d.matched_rule["rule_id"] == "P-01.3"


# ---------------------------------------------------------------------------
# P-09 regulatory hard limits (BDDK-style income-multiple cap)
# ---------------------------------------------------------------------------
def test_p09_statutory_cap_denies_excessive_total_limit():
    d = ENGINE.evaluate(ctx(
        customer={"branch_code": "B01", "hardship_flag": False,
                  "monthly_income_try": 30_000.0,
                  "current_limit_try": 100_000.0},
        request={"request_id": "REQ-TEST-P09", "increase_pct": 0.5,
                 "requested_increase_try": 50_000.0}))   # -> 5.0x income
    assert d.effect == "DENY" and d.matched_rule["rule_id"] == "P-09.1"
    assert "5.0" in d.rationale


def test_p09_within_cap_not_denied():
    d = ENGINE.evaluate(ctx(
        customer={"branch_code": "B01", "hardship_flag": False,
                  "monthly_income_try": 30_000.0,
                  "current_limit_try": 60_000.0},
        request={"request_id": "REQ-TEST-P09b", "increase_pct": 0.2,
                 "requested_increase_try": 12_000.0}))   # -> 2.4x income
